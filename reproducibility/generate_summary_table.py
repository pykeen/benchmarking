...
def main():
    for dataset, dataset_df in get_df().groupby('dataset'):
        ...
        x.columns = ['model'] + list(x.columns.get_level_values(1))[1:]
        ...
        table_latex = x.to_latex(
            index=False, escape=False, column_format='r' * len(x.columns)
        )
        ...
        write_latex(dataset, x)


def write_latex(dataset: str, df: pd.DataFrame) -> None:
    ...
    z = defaultdict(list)
    for measurement, y in df.columns[1:]:
    ...
    for measurement, y in z.items():
        ...
        table_latex = sub_df.to_latex(
            index=False,
            escape=False,
            column_format='r' * len(sub_df.columns),
            caption=f'{dataset} - {measurement}',
        )
        table_latex = table_latex.replace('±', '$\\pm$')
        tables.append((measurement, table_latex))

    with open(os.path.join(SUMMARIES, f'{dataset}.tex'), 'w') as file:
        print(template.render(tables=tables, dataset=dataset), file=file)
