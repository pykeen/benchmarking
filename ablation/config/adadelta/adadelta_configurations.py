...
def remove_adam_configurations(root, config, config_name, model_name_normalized):
    ...
    if ADAM in optimizers:
        config['ablation']['optimizers'].remove(ADAM)
        del config['ablation']['optimizer_kwargs'][
            MODEL_DIRECTORIES_TO_MODEL_NAME[model_name_normalized]][ADAM]

        del config['ablation']['optimizer_kwargs_ranges'][
            MODEL_DIRECTORIES_TO_MODEL_NAME[model_name_normalized]][ADAM]


def adapt_adadelta_configuration(config):
    ADADELTA = 'adadelta'
...
if __name__ == '__main__':
    ...
    for model_name_normalized, dataset, hpo_approach, training_assumption, config_name, path in iterator:
        ...
        adapt_adadelta_configuration(config=config)
